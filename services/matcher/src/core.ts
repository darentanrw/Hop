...
function getSealingKey() {
	return crypto
		.createHash("sha256")
		.update(
			process.env.MATCHER_SEALING_KEY ?? "dev-only-sealing-key-change-me",
		)
		.digest();
}
...
export async function submitDestination(address: string) {
	...
	if (!geocoded) {
		throw new Error(
			"Could not geocode the address. Please check the address and try again.",
		);
	}
	...
	store.destinations.set(sealedDestinationRef, {
		...
		routeDescriptorRef,
		normalizedAddress,
		maskedLocationLabel: buildMaskedLocationLabel(
			geocoded.postalCode,
			geohash5,
		),
...
export async function scoreRouteDescriptors(
	...
	for (let index = 0; index < routeDescriptorRefs.length; index += 1) {
		for (
			...
			const geohash6Match = areGeohashNeighbors(
				left.geohash6,
				right.geohash6,
			);
			const geohash5Match =
				!geohash6Match &&
				areGeohashNeighbors(left.geohash5, right.geohash5);

			if (!geohash6Match && !geohash5Match) {
				continue;
			}

			const spreadDistanceKm = haversineKm(
				left.lat,
				left.lng,
				right.lat,
				right.lng,
	...
	const tasks = eligiblePairs.map(
		(pair) => async (): Promise<CompatibilityEdge | null> => {
			...
			try {
				...
				const longestSingleTrip = Math.max(
					routeToLeft.timeSeconds,
					routeToRight.timeSeconds,
				);
				...
				detourMinutes = Math.max(
					0,
					(sequentialTrip - longestSingleTrip) / 60,
				);
			} catch {
				return null;
			}
			...
			const routeOverlap = Math.max(
				0,
				1 - detourMinutes / MAX_DETOUR_MINUTES,
			);
			const destinationProximity = Math.max(
				0,
				1 - spreadDistanceKm / MAX_SPREAD_KM,
			);
...
export function countDistinctLocations(
	members: Array<{ geohash6: string }>,
): number {
	...
	for (const member of members) {
		...
		for (const existing of seen) {
			if (
				member.geohash6 === existing ||
				areGeohashNeighbors(member.geohash6, existing)
			) {
				merged = true;
				break;
			}
...
export function revealEnvelopes(
	...
	for (const recipient of members) {
		...
		for (const sender of members) {
			const destinationRecord = store.destinations.get(
				sender.sealedDestinationRef,
			);
			if (!destinationRecord) {
				throw new Error(
					`Missing matcher destination record for ${sender.sealedDestinationRef}.`,
				);
			}
...
function getDestinationRecord(sealedDestinationRef: string) {
	...
	if (!record) {
		throw new Error(
			`Missing matcher destination record for ${sealedDestinationRef}.`,
		);
	}
	return record;
}
...
async function findOptimalOrder(
	members: PreviewRider[],
): Promise<PreviewRider[]> {
...
export async function buildSimulatorPreview(
	...
	const riderById = new Map(
		riderPreviews.map((rider) => [rider.riderId, rider]),
	);
	const groups = [];
	...
	for (const group of request.groups) {
		...
		if (memberRiders.length !== group.members.length) {
			const missing = group.members.find(
				(m) => !riderById.has(m.riderId),
			);
			throw new Error(
				`Missing matcher preview rider for ${missing?.riderId}.`,
			);
		}
