...
const matcherBaseUrl =
	process.env.NEXT_PUBLIC_MATCHER_BASE_URL ?? "http://localhost:4001";
...
function buildAddressEntriesFromRequest(
...
function buildRequestFromControls(args: {
	...
	const { windowStart, windowEnd } = slotsToIsoRange(
		args.dateInput,
		args.startSlot,
		args.endSlot,
	);
...
export function AdminSimulatorClient() {
		...
		const nextEntries = result.riders
			...
			.map((rider) => {
				const group = result.groups.find(
					(entry) => entry.groupId === rider.groupId,
				);
				return {
					id: createId("log"),
	...
	const generatedJson = useMemo(
		() => toSimulatorJson(generatedRequest),
		[generatedRequest],
	);
	const { dateLabel, timeLabel } = formatRangeSummaryParts(
		dateInput,
		startSlot,
		endSlot,
	);
	...
	function addCurrentAddress() {
		const nextAddress = (
			selectedSuggestion?.address ?? addressQuery
		).trim();
		if (!nextAddress) {
		...
		appendLog(
			`+ ${selectedSuggestion?.title ?? formatAddressSnippet(nextAddress, 50)}`,
			"success",
		);
	...
	function loadSample() {
		...
		resetSearchBox();
		appendLog(
			`Loaded ${sampleAddresses.length} sample addresses.`,
			"success",
		);
	}
	...
	function clearAddresses() {
		...
		resetSearchBox();
		replaceLogs([
			{ id: createId("log"), message: "Cleared.", tone: "warning" },
		]);
	}
	...
	async function runSimulation() {
		...
		try {
			...
			setActivityLog((current) => [
				...current,
				{ id: createId("log"), message, tone: "danger" },
			]);
		} finally {
			setRunning(false);
		}
		...
		<div className="page-container no-nav admin-simulator-page">
			<div className="stack-lg stagger admin-simulator-shell">
				...
				<section className="admin-simulator-workbench">
												...
												<div className="admin-compat-bar-row">
													...
													<div className="admin-compat-bar-track">
														<div
															...
															style={{
																width: `${edge.routeOverlap * 100}%`,
															}}
														/>
													</div>
													<span className="text-xs font-mono text-muted">
														{edge.routeOverlap.toFixed(
															2,
														)}
													</span>
												</div>
												<div className="admin-compat-bar-row">
													...
													<div className="admin-compat-bar-track">
														<div
															...
															style={{
																width: `${edge.destinationProximity * 100}%`,
															}}
														/>
													</div>
													<span className="text-xs font-mono text-muted">
														{edge.destinationProximity.toFixed(
															2,
														)}
													</span>
												</div>
						...
						>
							...
							<div className="admin-sim-addr-list">
								{addresses.map((entry, index) => (
									<div
										className="admin-sim-addr-row"
										key={entry.id}
									>
										<span className="admin-sim-addr-idx">
											{index + 1}
										</span>
										<span
											className="admin-sim-addr-text"
											title={entry.address}
										>
											{entry.verifiedTitle ??
					...
					<section className="card stack admin-simulator-map-stage">
						...
						<div className="admin-simulator-map-frame">
							<AdminSimulatorMap
								result={result}
								visibleRiderCount={visibleRiderCount}
							/>
						</div>
					</section>
						...
						<div className="card" style={{ padding: 14 }}>
							...
							<div className="admin-sim-stats">
								<div className="admin-sim-stat">
									...
									<span className="admin-sim-stat-k">
										Riders
									</span>
								</div>
								<div className="admin-sim-stat">
								...
								<div className="admin-sim-stat">
									<span className="admin-sim-stat-v">
										{result?.stats.unmatchedRiders ?? 0}
									</span>
									<span className="admin-sim-stat-k">
										Unmatched
									</span>
								...
								<div className="admin-sim-stat">
									<span className="admin-sim-stat-v">
										{result?.stats.averagePairScore?.toFixed(
											2,
										) ?? "—"}
									</span>
									<span className="admin-sim-stat-k">
									...
									{result.groups.map((group) => (
										...
										>
											<div className="row-between">
												...
												>
													avg{" "}
													{group.averageScore.toFixed(
														2,
													)}{" "}
													·{" "}
													{group.maxDetourMinutes.toFixed(
														0,
													)}
													m detour
												</span>
				...
				<details className="card stack">
					...
					<pre className="admin-simulator-json-preview">
						{generatedJson}
					</pre>
				</details>
			</div>
			...
			{showImportModal ? (
				<div
					...
					onKeyDown={(e) =>
						e.key === "Escape" && setShowImportModal(false)
					}
				>
					<div
